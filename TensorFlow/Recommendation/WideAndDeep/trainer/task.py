...
from trainer import criteo_features
from utils.criteo_dataloader import input_fn
...
def create_parser():
    ...
    # model type, checkpoints and network architecture
    parser.add_argument(
        '--model_type',
    ...
    # epoch, batch size, evaluate steps
    parser.add_argument(
        '--global_batch_size',
...
def main(FLAGS):
    ...
    def deep_optimizer():
        ...
        return opt
    print("****************************************************************************")
    # wide columns, deep columns
    wide_columns, deep_columns = criteo_features.get_feature_columns()
    ...
    train_input_fn = lambda: input_fn(data_file='dataset/criteo/whole_train.tfrecord',
                                      num_epochs=FLAGS.num_epochs,
                                      shuffle=False,
                                      batch_size=FLAGS.global_batch_size)
    eval_input_fn = lambda: input_fn(data_file='dataset/criteo/whole_test.tfrecord',
                                     num_epochs=1,
                                     shuffle=False,
                                     batch_size=FLAGS.eval_batch_size)
    print("********************************************************************************")
    # estimator, wide and deep model
    ...
    if FLAGS.predict or FLAGS.evaluate:  # inference
        ...
        if FLAGS.benchmark:
            ...
            if result != (None, None):
                dllogger.log(step=(), data={"result": result})
